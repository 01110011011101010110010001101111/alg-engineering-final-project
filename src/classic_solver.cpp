...
void read_regions_from_file(const std::string& filename, std::map<int, MatrixXd>& As, std::map<int, VectorXd>& bs, int& n_regions) {
    ...
    file >> n_regions;
    LOG("Read n_regions: " << n_regions);
    for (int region = 0; region < n_regions; ++region) {
        int rows, cols;
        file >> rows >> cols;
        LOG("Region " << region << " matrix size: " << rows << "x" << cols);
        ...
        int b_size;
        file >> b_size;
        LOG("Region " << region << " vector size: " << b_size);
        VectorXd b(b_size);
        for (int i = 0; i < b_size; ++i) {
            if (file.eof()) {
                std::cerr << "Unexpected end of file while reading b vector for region " << region << std::endl;
                exit(1);
            }
            file >> b(i);
            LOG("b[" << i << "] = " << b(i));
        ...
        if (b.size() != rows) {
            std::cerr << "Size mismatch: b.size() = " << b.size() << ", A.rows() = " << rows << " in region " << region << std::endl;
            exit(1);
        }
        As[region] = A;
        bs[region] = b;
...
int main(int argc, char* argv[]) {
    ...
    int n = 4;
    std::string input_filename = "output.txt"; // Change if needed
    read_regions_from_file(input_filename, As_int, bs_int, n);
    ...
    int s_idx = n;
    int t_idx = n + 1;
    As_int[s_idx] = A_s; bs_int[s_idx] = b_s;
    As_int[t_idx] = A_t; bs_int[t_idx] = b_t;
